...
{
    public class BLL_DeliveryAddress
    {
        ...
        public DataTable Select(string id)
        {
            ...
            if (id == null)
            {
                cmd.CommandText = "SELECT * FROM deliveryAddress";

            }
            else
            { 
                cmd.CommandText = "SELECT * FROM deliveryAddress where DeliveryAddress='"+id.ToString()+"'";
        ...
        public string Insert()
        {
            ...
            try
            {
                ...
                {
                    using (OracleCommand cmd = new OracleCommand("Insert into DeliveryAddress(DeliveryAddressId, Latitude, Longitude,DeliveryPoint,orderNumber)Values('" + this.DeliveryAddressId + "'," + this.Latitude  + "," + this.Longitude +",'"+this.DeliveryPoint+ "','"+this.OrderNo+"')"))
                    {
                        cmd.Connection = con;
                        con.Open();
                        cmd.ExecuteNonQuery();
                    ...
                    //using (OracleCommand cmd = new OracleCommand("update Orders set deliveryAddressId='"+DeliveryAddressId+"' where orderNumber ='"+OrderNo+"');"))
                    //{
                    //    cmd.Connection = con;
                    //    con.Open();
                    //    cmd.ExecuteNonQuery();
        ...
        public string Update()
        {
            try
            {
                ...
                using (OracleConnection con = new OracleConnection(constr))
                {
                    using (OracleCommand cmd = new OracleCommand(@"update DeliveryAddress set
                                                    orderNumber = '" + this.OrderNo +
                                                        "',deliveryPoint= '" + this.DeliveryPoint +
                                                        "',latitude= " + this.Latitude +
                                                        ",longitude= " + this.Longitude +
                                                        " where DeliveryAddressId = '" + this.DeliveryAddressId + "'"))
                    {
        ...
        public string Delete()
        {
            ...
            try
            {
                ...
                using (OracleConnection con = new OracleConnection(constr))
                {
                    ...
                    using (OracleCommand cmd = new OracleCommand("DELETE FROM DeliveryAddress WHERE DeliveryAddressId ='" + this.DeliveryAddressId + "'"))
                    {
